"""Data Quality Validation Skill inspecting DDL nullability and default values."""
...
class DataQualitySkill(BaseSkill[SchemaValidationInput, DataQualityValidationResult]):
    """Semantic constraints review inspecting nullability, defaults, and uniqueness constraints."""

    name = "data_quality"
    version = "1.0.0"
    input_schema = SchemaValidationInput
    output_schema = DataQualityValidationResult

    def __init__(self, gateway: LLMGateway | None = None) -> None:
        """Initialize DataQualitySkill.
    ...
    async def prepare(
        ...
        """Load and render data quality prompt template.
    ...
    async def execute(self, prepared_data: Any, _context: SkillContext) -> Any:
        ...
        """
        contract_request = AIContractRequest[DataQualityValidationResult](
    ...
    async def post_process(
        ...
        Returns:
            DataQualityValidationResult: Validated Pydantic model.
        """
        if not execution_result.success:
            raise SkillExecutionError(
                f"Data Quality Skill contract execution failed: {execution_result.error.message if execution_result.error else 'Unknown error'}"
            )
        from typing import cast

        return cast(DataQualityValidationResult, execution_result.data)
