"""Naming Validation Skill inspecting database naming standards."""
...
class NamingSkill(BaseSkill[SchemaValidationInput, NamingValidationResult]):
    """Semantic style review verifying table, column, and constraint naming."""

    name = "naming"
    version = "1.0.0"
    input_schema = SchemaValidationInput
    output_schema = NamingValidationResult

    def __init__(self, gateway: LLMGateway | None = None) -> None:
        """Initialize NamingSkill.
    ...
    async def prepare(
        ...
        """Load and render naming prompt template.
    ...
    async def execute(self, prepared_data: Any, _context: SkillContext) -> Any:
        ...
        """
        contract_request = AIContractRequest[NamingValidationResult](
    ...
    async def post_process(
        ...
        Returns:
            NamingValidationResult: Validated Pydantic model.
        """
        if not execution_result.success:
            raise SkillExecutionError(
                f"Naming Skill contract execution failed: {execution_result.error.message if execution_result.error else 'Unknown error'}"
            )
        from typing import cast

        return cast(NamingValidationResult, execution_result.data)
